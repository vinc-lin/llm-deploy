...
| **Qwen3-VL-4B — text tower** | ❌ **ROOT-CAUSED 2026-08-21: the model is fed a prompt form it was never calibrated for.** `vl_calib_build.py` builds EVERY calibration window with `apply_chat_template(..., add_generation_prompt=True)`, so the 4B's activation ranges assume position 0 is `<\|im_start\|>` and the sink is at row 1. Every 4B text test we have run feeds a **raw** prompt — the v5 guide's Test 3a is `genie-t2t-run -p "What is 2+2? …"`, and `genie_dialog_qwen3vl_4b.json` sets `bos-token: -1` with **no prompt template**. Raw puts the sink at row 0, out of distribution → clipping → position-0 hidden state and KV corrupted in both shards → everything attending to the sink degenerates. **Device confirmation pending: `docs/TEST_I_templated_prompt.md`, one minute** |
