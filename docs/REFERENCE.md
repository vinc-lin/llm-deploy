...
| ↳ **ROOT-CAUSED 2026-08-20** | **Activation-encoding range clipping**, not fp16 saturation — `docs/ROOTCAUSE_qwen3vl_4b_boundary_gain.md`. Test F returned CONDITION (gain follows the sink to row 4, vanishes when row 0 gets context). `quantize_aimet.py:475` hard-codes `post_training_tf_enhanced`, an MSE-optimal range search that deliberately discards outliers, so sink-row activations land outside their calibrated INT16 range: `layers.0/mlp/down_proj` **9.00 vs 5.482** (1.64× over), `layers.1/mlp/Mul_1` **61.40 vs 59.272**. Clamping those two in the host fp32 graph reproduces the device exactly — **gain 1.3914 vs 1.38959, residual 0.416% vs 0.447%, cosine 0.999991 vs 0.999990**, rows 1–3 clean in both. ⚠ Neither tensor alone does it (0.93× and 1.06×); the interaction does |
| ↳ the fix | Make the activation quant scheme selectable (hard-coded today), requantize the text tower with min/max or high-percentile activations, gate on `scan_activation_clipping.py` (nothing over range on a sink row) + `sim_activation_clipping.py` (row-0 gain back to ~1.00), rebuild the two ctx-bins. Weights are unaffected. **Not yet built.** Unchecked: whether the 0.6B and the ViT clip too — same scan answers it |
| ↳ why every gate missed it | cosine is scale-invariant, and the host parity harness validates ONNX numerics without ever executing the quantized activation path. The two scans above are the new gate |
