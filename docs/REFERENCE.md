...
| ↳ next measurement | **Test F, shipped 2026-08-20** — `docs/TEST_F_sink_vs_position.md`. Separates the sink CONDITION from the ROW INDEX: `f1_row0ctx` keeps the index and removes the sink (host row-0 RMS 107.20 → 1.00), `f2_shift4` keeps the sink and moves it to row 4 (bit-identical to the control's row 0 on the host, `max\|diff\| = 0.000e+00`). No rebuild — same ctx-bins the device already has. Also picks up the 36 per-layer KV taps shard 0 has written and discarded on every run since v5 |
