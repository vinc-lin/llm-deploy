...
| **Qwen3-VL-4B — text tower** | ❌ **still the only blocker, and now localised DOWNSTREAM of shard 0.** Test G (device, 2026-08-20, `reports/qwen3vl-4b-testg-device-results-2026-08-20.md`) measured the boundary on realistic chat-templated prompts: **worst \|gain−1\| 4.4%**, real sink (row 1, RMS 220.3) at **0.989 / cos 1.000000**, decode-with-real-cache at **0.991**, all logits argmax matching both chained and isolated. **Shard 0's W8A16 is faithful on production input — the boundary line of enquiry is CLOSED.** History below |
| ↳ (history) the 1.39× | ❌ was **the only remaining blocker.** One defect: shard 0's boundary output carries a **uniform 1.38959× gain on row 0 alone** (residual 0.447%, cosine 0.999990 — cosine is scale-invariant, which is why every earlier gate passed it). Rows 1–3 are clean. Reproduces under bare `qnn-net-run`, so it is **not** a Genie fault. Any uniform gain in [1.25, 3.0] reproduces the device's wrong argmax 105196 exactly, including the row pattern — `docs/TEST_E_boundary_scale.md` |
| ↳ what row 0 is | the **attention-sink** row: RMS 107.22 vs 2.26/1.26/1.20; c4 = 5244 is 93.44% of its squared norm and c4² = 2.75e7 overflows fp16's 65504 by **420×**. `decode1tok` shows the identical 1.3896× and shares exactly one property — a token attending only to itself |
| ↳ dead ends (do not re-spend) | VL deepstack injection (all six inputs are **exactly zero**, 0 nonzero elements — the proposed "zero them out" test is a no-op); a single bad channel (c4-alone predicts cosine 0.9973 vs 0.999990 measured); KV bookkeeping, LUT requantization, Genie orchestration, boundary dtype/encodings. The QDQ double-quantization theory from the device team's v4 §2 is also falsified — 0 Q/DQ ops in either ONNX |
| ↳ the 1.39× is EXPLAINED — **for the probe only** | **Activation-range clipping**, not fp16 saturation — `docs/ROOTCAUSE_qwen3vl_4b_boundary_gain.md`. Test F returned CONDITION. The probe's sink row lands outside its calibrated INT16 range: `layers.0/mlp/down_proj` **9.00 vs 5.482**, `layers.1/mlp/Mul_1` **61.40 vs 59.272**. Clamping both in the host fp32 graph reproduces the device — **gain 1.3914 vs 1.38959, residual 0.416% vs 0.447%, cosine 0.999991 vs 0.999990**. ⚠ Neither alone does it (0.93× and 1.06×); the interaction does. So the clamp sim is a faithful device stand-in, good to 0.3%, with no hardware |
| ↳ ⚠ **but it does NOT fire in production** | The probe's row 0 is a bare word token at position 0; a real prompt is chat-templated and starts `<\|im_start\|>`. Same clamp sim on **realistic windows**: row-0 gain **0.9990**, worst over rows 0–3 across six windows **0.035** — against 0.390 on the probe. The real attention sink sits at **row 1, RMS 220.3** (bigger than the probe's 107.2) and comes through at **1.0000**. Held-out eval windows exceed their ranges by at most 1.082×. **Tests B/C/E/F have been measuring an artifact of the probe's synthetic token sequence** (`scan_clipping_realistic.py`, `sim_clipping_realistic.py`) |
| ↳ two dead fixes, neither built | "fp16 saturation" — these activations are INT16 with calibrated ranges, and the arithmetic misses by 15×. "`tf_enhanced` discards the outlier" (**our own claim, falsified same day**) — the ranges sit ON the calibration max (5.4819 vs 5.4805 observed), so nothing was discarded and **a min-max requantize would produce identical ranges**. `--act-range-scheme` was added and is correct, but changes nothing here |
| ↳ where that leaves production | The activation path is clean on realistic inputs to within 3.5%, which **rules the quantized activation ranges out** as the production cause — a real narrowing, but the garbled text is **back to unexplained**. Next: rebuild the probe kit from chat-templated prompts (the calib/eval windows already are this) and run it on device. Host predicts a clean boundary; a dirty one there would put the fault in the ctx-bin/converter, which the clamp sim cannot see |
| ↳ **Test G shipped 2026-08-20** | `docs/TEST_G_realistic_prompt_boundary.md` — the first probe fed production-shaped input: **four** chat-templated cases (three held-out) with real ViT features, deepstack zeroed as shipped. No rebuild; same bins already on device. **Ships a falsifiable host prediction**: every reference row within ±5%, worst **1.0347**, real sink (row 1, RMS 220.3) at **1.0002**. Clean ⇒ the boundary enquiry closes and the fault is downstream (Genie feed / dialog path / shard 1); dirty ⇒ the ctx-bin or converter, the one stage the host clamp sim cannot model |
| ↳ `r3_decodectx` — generation's own path | Every earlier decode probe fed an **empty, fully-masked cache**, which makes the token attend only to itself, i.e. the very sink condition that manufactured the artifact. `r3_decodectx` seeds a real 13-position cache from a real prefill and takes one greedy step. Boundary RMS **0.936, not 107** — a decode step with real context is not a sink. Clamping every calibrated activation range moves it by **nothing** (0.9999 clamped and unclamped, cos 1.000000), so this path has no exposure to the clipping class at all. Its 72 cache tensors are 320.7 MB raw, >99% zero, shipped as a 3 MB tar.gz the operator expands host-side; the runner size-checks them and stops with the tarball hint if missing (`PAST_MODE=files`) |
| ↳ **what remains, ranked** | **(a) multi-chunk prefill** — a 273-token prompt is three AR=128 calls and chunks 2–3 run against a *partially populated* cache; **every probe to date has run a single chunk against an empty cache**, and that is exactly where the FLOAT_16 padding bug lived. **(b)** Genie's multi-shard orchestration and its own KV management (the probe does a file handoff; Genie chains in memory). **(c)** dialog/config — rope-theta, MRoPE. **Largely answered:** the LUT *precision* path (Test G fed the identical uFxp_16 grid Genie writes) and shard 1 (isolated logits correct) |
| ↳ **Test H shipped 2026-08-20** | `docs/TEST_H_multichunk_prefill.md` — the **cross-chunk prefill path, never once run on device**. Every probe including G ran a single chunk against an empty cache; a real 273-token prompt is three AR=128 calls carrying KV forward. Three cases from one real image turn: `c0_chunk0` (128 rows, empty cache — anchor, reproduces Test G's `r2_chunk0` exactly: argmax 3175, RMS 2.072/220.330/1.117/1.428/0.839), `c1_chunk1` (128 rows, 128-position cache), **`c2_chunk2` (21 rows, 256-position cache — the PARTIAL last chunk, i.e. the exact `variant > n_process` condition the FLOAT_16 padding bug fired on)**. Host predicts every row within 0.0008 of 1.0; read as a lower bound, ±5% is a PASS |
| ↳ ⚠ HF proxy transfer limit (measured) | The proxy commits ~190 small files fine but **drops `upload_folder`'s batched transfer** — a 53 MB file failed 3× (520/290/224 s) and 8 MB parts failed too, yet a single `upload_file` of a 2 MB part committed instantly because the blob was already server-side. **The batching is the problem, not the size.** Working recipe: `preupload_lfs_files` per file with per-file retry, then one `create_commit` — 190/190 preuploaded, one commit, first try. Test H's 594 MB cache therefore ships as 27 × 2 MB parts plus an md5 of the whole |
| ↳ ⚠ prediction calibration | `sim_activation_clipping.py` predicted 3.5% and the device measured 4.4%. It applies `Clip` but does **not** round to the quantization grid, so it models clipping and omits rounding noise. **Read its predictions as lower bounds on deviation, not point estimates** |
| ↳ method note | The probe was built as "the simplest thing that can fail", and that is what made it mislead: the simplest input is not a typical one, and a synthetic-only defect still reproduces perfectly on hardware. Check every probe against a realistic input before generalising its result |
