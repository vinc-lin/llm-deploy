...
| **Qwen3-VL-4B — text tower** | ❌ **the only remaining blocker.** One defect: shard 0's boundary output carries a **uniform 1.38959× gain on row 0 alone** (residual 0.447%, cosine 0.999990 — cosine is scale-invariant, which is why every earlier gate passed it). Rows 1–3 are clean. Reproduces under bare `qnn-net-run`, so it is **not** a Genie fault. Any uniform gain in [1.25, 3.0] reproduces the device's wrong argmax 105196 exactly, including the row pattern — `docs/TEST_E_boundary_scale.md` |
