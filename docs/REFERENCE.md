...
| Qwen3-VL-4B, v3 (multimodal) | Text tower rebuilt with grouped attention (`--grouped-gqa` now reaches `from_hf_vl_text`) and image blobs padded to 3,149,824 B. All device-free gates pass — `lint_gqa_ops.py --layers 18` 0/4 failing, full 6-chain `parity_e2e_vl.py` (device path `chain0b-prefillkv` 20/20), Genie load-sim PASS. Superseded by the v4/v5 rows below — `reports/qwen3vl-v3-gate-results.md` (correction #34) |
| **Qwen3-VL-4B — image path** | ✅ **working on device.** The v3 SIGSEGV was Genie reading the image file as fp32 regardless of tensor dtype (a UFixed16 blob is over-read 2×, ~3 MB, into the guard page). v4 ships `*_fp32.raw` (6,295,552 B): 3 clean pipeline runs + 6 real photographs, no crash. ViT correct under both `qnn-net-run` and Genie |
| **Qwen3-VL-4B — text tower** | ⚠ **root cause CONFIRMED on device 2026-08-21 (Test I) — and a SECOND, separate defect exposed behind it.** Templated prompt → first token **correct** (`4`); raw → wrong. Graph level: `i0_raw` row 0 **1.390**, `i1_templated` clean with **5/5** chained argmax. The calibration/prompt-form mismatch is settled. But generation then degenerates into a repetition loop — see below. `reports/qwen3vl-4b-testi-device-results-2026-08-21.md` |
| ↳ ⚠ **the repetition IS a defect** | Not greedy-without-EOS. `eos-token: [151645, 151643]` is configured with greedy sampling, and the host, running the real split graphs with the full KV recurrence, generates **exactly two tokens: `4` + `<|im_end|>`, then stops** (`scripts/validate/host_generate_check.py`). **The prior note in `ISSUE_qwen3vl_4b_text_numerics.md` §2.2 — "4 then repeats is expected" — is WITHDRAWN as false**, including for the 0.6B it was written about |
| ↳ the failure is at **decode step 1** | The first generated token comes from **prefill**, not decode. So prefill is right (device `4`, and `i1_templated` 5/5 argmax) and the **very first decode call** is already wrong — it should emit `151645 <\|im_end\|>` and emitted `2939 'ention'`. Tighter than "fails after token 1" |
| ↳ ⚠ correction: shard 1 decode IS tested | Test G's `r3_decodectx` ran a decode step on a real 13-position cache through **both** shards: chained argmax 1/1 cos 0.99994, isolated 1/1 cos 0.99998. Together with the host run and `parity_e2e_vl` 20/20 vs HF, the decode **graphs** and the KV contract are verified. "Shard 1 decode never tested" is not correct |
| ↳ **Test J (device, 2026-08-21) — decode graphs CLEARED** | `reports/qwen3vl-4b-testj-device-results-2026-08-21.md`. Stage A1 ran the decode step under `qnn-net-run` with a host-built real cache: `j0`→**151645**, `j1`→**9104**, `j2`→**4344**, all 1/1 chained and isolated, worst cos 0.9998, shard-0 boundary worst \|gain−1\| **0.0061**. `j2`'s cache contains a row **written by the decode graph itself** — so the recurrence is device-proven too. Stage B reproduced the Genie-side symptom verbatim (templated first token right, then a repetition loop; raw wrong from token 0). ⇒ **the fault is entirely in Genie's decode-step feed** |
| ↳ ✝ two of the three candidates are DEAD (device-free, 2026-08-21) | **(1) the prefill→decode KV width handoff is device-proven**: the shipping 0.6B `gqafix-ladekv` makes the same 1024→1151 change with the same `[1,8,128,PAST]` key layout on the same libGenie and generates correct coherent text in basic mode. The Test I ranking that put this first is **withdrawn**. **(3) MRoPE decode advance is not involved in a text-only run** — `nsp-model.cpp:3803` gates the Qwen3-VL branch on `m_visionParam.size() > 0`, so Stage B was plain rope. (Also checked clean: the rope table is filled for the whole `m_ctx_size`, and `setVisionParam` sets `m_ropeInitialized = false` at `:4309`, so a changed vision param **does** rebuild it) |
| ↳ what is actually left — **two candidates** | (a) the **external-LUT `inputs_embeds` feed** at decode time, and (b) the **two-ctx-bin split** — shard 0 hands `[1,1,2560]` to shard 1 on every decode step. These are the only structural differences left between the 0.6B that works and the 4B that does not |
| ↳ **Test K (device, 2026-08-21) — image path VERIFIED, LUT feed NEWLY IMPLICATED** | `reports/qwen3vl-4b-testk-device-results-2026-08-21.md`. **K2: the image → ViT → splice → 3-chunk prefill path is correct on device for 7/7 images.** Scored on first tokens against an HF fp32 reference (`host_first_token_vl.py`), not rendered strings: `Red`, `Rain`, `Snow`, `Snow` are single tokens and come out whole; `Sunny` is `['S','unny']`, so `wx_clear`/`wx_clear2` show the **correct token 0** and then the decode defect (`Sally` = `['S','ally']` catches it red-handed). ⚠ The device team's 3/7 score and its attribution of the failures to the `c1` cross-chunk anomaly are both wrong — all seven prompts are 279–281 tokens, i.e. **three chunks each**, so chunking cannot be the splitting variable; answer *tokenization* is |
| ↳ ⛔ **K1 is VOID — it ran a FLOAT_16 bin (2026-08-22)** | The bundle published for Test K shipped the **pre-graft** ctx-bin (`880a6abd…`, `inputs_embeds` = `FLOAT_16`, **fails `lint_embedding_dtype.py`**); the corrected build is `9720e46e…` (`UFIXED_POINT_16`) and **passes**. That dtype is the documented pad-write-overwrites-the-prompt defect (`nsp-model.cpp:3144` vs `:1813`), and it fires on every one of the probe's prompts (12/5/6 tokens in an **AR=128** graph ⇒ `variant > n_process` always). It explains the symptom exactly: prompt front survives, back destroyed, **decode untouched** (AR=1). ⇒ **Genie's LUT feed was never tested** — neither exonerated nor implicated — and the **split** is again the sole live suspect for the 4B. Re-run is **Test L** (`TEST_L_ctxbin_vs_genie.md`). Correction #39. The row below records what K1 measured and stays true as *measurement*; only its attribution was wrong |
| ↳ ⚠ **K1's verdict INVERTS under a host reference** | The team read probe-vs-control with no reference and concluded the probe looked healthier ⇒ LUT sound ⇒ split at fault. HF fp32 on the same raw prompts reproduces the **control** almost exactly (` 2+2=4. 2+2=4. …`, first ids `[220,17,10,17,28,19]`) — **the control's repetition is correct model behaviour, not a deficiency.** The **probe** diverges at **token 0** (`</think>` 151668 vs `Ġ` 220) and answers a mountain-weather question with 64 fluent tokens about New York. And `parity_lutprobe.py` passes **3/3 vs HF** on the shipped probe's own graph + LUT — the gate whose docstring says a device failure after it "implicates the runtime". Eliminated as causes: fp16 `inputs_embeds` (it is `uFxp_16`, grafted), `graph_names` (both graphs covered, vtcm/spill read back from the bin), tensor naming, tokenizer (**byte-identical** md5 `6423133b…`), quantization noise (the control is also W8A16 and matches HF) |
| ↳ what K1 established — two things, opposite ways | (1) **the probe's decode is healthy** — 64 fluent tokens, clean EOS; it does *not* reproduce the 4B's collapse, so LUT feeding does not break *decode*; (2) **the probe's prefill is wrong on device** with graph and LUT proven correct on the host ⇒ **Genie's LUT feed is newly implicated**. ⚠ Hypothesis (not a finding): Genie's LUT feed delivers a degraded `inputs_embeds` — the 4B's Genie-level prefill evidence is only **one robust token** (`4`/`Mountain`), since `i1_templated`'s 5/5 was measured under `qnn-net-run` with host-supplied files, which bypasses the LUT entirely. Decisive test: run the **probe's ctx-bin under `qnn-net-run`** — argmax 220 ⇒ Genie's feed; argmax 151668 ⇒ a converter defect |
| ↳ **4B timing, first numbers (K3)** | Two-shard W8A16, 18-token text prompt: **init ~1375 ms cold, TTFT 510 ms, prefill 35.3 tok/s, decode 6.5 tok/s.** Internally consistent (18 ÷ 35.3 = 510 ms). Decode ≈ 0.6B's 43 tok/s ÷ 6.67 (the parameter ratio) — compute-bound, agreeing with correction #27. **Consequence:** at 35.3 tok/s prefill a 279-token image prompt costs **~7.9 s** before the first token, which is what the 8–12 s per-image wall-clock is. ⚠ Warm init (1725 ms) > cold (1375 ms) is unexplained |
| ↳ **next test — already built, already on the Hub, never run** | `docs/DEVICE_TEST_qwen3_06b_lut.md` — `qwen3_06b_lutprobe`, a 0.6B on the shipping ladekv recipe with **one** ctx-bin and a LUT feed: one variable. Matches the control ⇒ the feed is sound and **the split is the fault** by elimination; first token right then diverges ⇒ **the LUT feed is the fault**, reproducible in a 1.4 GB bundle instead of 4.5 GB. ⚠ Pass criterion is **tokens matched against the working 0.6B run back-to-back**, not "is it coherent" — the control repeats too |
| ↳ ⚠ cross-chunk prefill (`c1`) — anomalous, NOT yet a defect | Test J Stage A2: `c0` clean, **`c1` row 2 gain 0.9226 / cos 0.960, chained 3/5 vs isolated 5/5**, `c2` clean at 5/5. Real by the kit's own `GAIN_TOL = 0.05` and localised to shard 0 — but the "broken cross-chunk cache path" reading is unsupported: `c2` carries a **larger** cache and is cleaner; the sampled rows are confounded (`c0`'s clean rows 0–3 are **text**, `c1`'s dirty rows are **all image-feature**, ±11.6 vs ±0.24); the reference uses an **fp32** cache against the device's quantized one; and these logits are discarded in production anyway. ⚠ `build_chunk_suite` is a **host** replay — `c2` was fed the host's clean cache, never `c1`'s output — so "the error recovered in the final chunk" is false and **propagation is unmeasured** |
| ↳ ⚠ `genie-t2t-run` flags (measured 2026-08-21) | **`--max-num-tokens` does not exist.** It is the dialog-config key `dialog.max-num-tokens` (`Dialog.cpp:2493`, read at `:2888`). Passing the flag aborts the run; omitting the cap runs to `Context Size was exceeded`, which frees the handle and **writes no profile** — that cost all 4 profiles in the Test J session and 7 of 8 in an earlier one. Also: **`--profile` refuses an output file that already exists** (`main.cpp:528-533`), so re-runs need a fresh name. **`-tok`/`--tokens_file` does exist** — prefer it to settle tokenizer questions by construction |
| ↳ the controlled measurement | Same question, template the only variable (`--suite i`, measured 2026-08-21). **raw**: sink at row 0, RMS 107.202, `layers.0/mlp/down_proj` **9.00 vs 5.482 = 1.64× over**, boundary row-0 gain **1.3505** — inside the [1.25, 3.0] band that reproduces the device's wrong argmax. **templated**: sink at row 1, RMS 220.330, worst overshoot **1.01×**, boundary gain **1.0001**, every row ≤ 1.0011 |
| ↳ why the 0.6B never showed it | Its calibration used **raw** prompts (`CALIB_PROMPTS` in `quantize_aimet.py`). Same code, opposite input contract — so "the 0.6B works, therefore the pipeline is fine" was misleading for weeks |
| ↳ what this explains | Test C's ladder (n=1 wrong from the poisoned row 0; n≥4's first token right because it comes from the last prompt row; later steps garbage off the corrupted position-0 KV), Test F's layer scan diverging from layer 1 on sink rows, photos collapsing to EOS while the synthetic sample repeated, and host parity 20/20 (it feeds templated windows, so it exercises the in-distribution path and cannot see this) |
| ↳ the fix | **Template the prompt.** `bos-token: -1` is already correct — the template supplies `<\|im_start\|>` itself. The `genie-app` image path must use `node set textFile` (script strings never unescape `\n`). Only if raw prompts must ALSO work: add untemplated windows to `vl_calib_build.py` and requantize — the one case where `--act-range-scheme` matters. **No rebuild otherwise** |
| ↳ Test G (device, 2026-08-20) | `reports/qwen3vl-4b-testg-device-results-2026-08-20.md` — on chat-templated prompts the device boundary is clean: worst \|gain−1\| **4.4%**, real sink at **0.989 / cos 1.000000**, decode-with-real-cache **0.991**, all logits argmax matching. **Shard 0 and shard 1 are exonerated on in-distribution input.** Read now as: the in-distribution half of the Test I pair, confirmed on hardware |
| ↳ Test H — still worth running, but NOT the top suspect | `docs/TEST_H_multichunk_prefill.md`, shipped 2026-08-20: the cross-chunk prefill path, never once run on device (`c2_chunk2` is the partial-last-chunk `variant > n_process` condition). ⚠ It **cannot explain the observed garbage** — those prompts are ~15 tokens, i.e. single-chunk. Ranking it as the strongest suspect was an error of the retracted v2 root cause |
| ↳ ⚠ HF proxy transfer limit (measured) | The proxy commits ~190 small files fine but **drops `upload_folder`'s batched transfer** — a 53 MB file failed 3× (520/290/224 s) and 8 MB parts failed too, yet a single `upload_file` of a 2 MB part committed instantly because the blob was already server-side. **The batching is the problem, not the size.** Working recipe: `preupload_lfs_files` per file with per-file retry, then one `create_commit` — 190/190, one commit, first try |
| ↳ ⚠ prediction calibration | `sim_activation_clipping.py` predicted 3.5% and the device measured 4.4%. It applies `Clip` but does **not** round to the quantization grid. **Read its predictions as lower bounds on deviation, not point estimates** |
| ↳ method note (corrected) | The earlier lesson here — "the simplest input is not a typical input" — was itself half wrong. The probe (`[3838, 374, 264, 1273]` = "What is a test") was a **faithful miniature of what our device tests actually feed**; Test C's `prompt_4.txt` is that exact string. The real failure was checking the probe against an *imagined* production instead of **what our tests execute**, and never checking which prompt form the calibration used. **Verify the input contract between calibration and inference before trusting any probe result** |
| ↳ test tooling | `docs/TEST_F_sink_vs_position.md` — the sink-vs-index probe kit, and the 36 per-layer KV taps shard 0 had been writing and discarding on every run since v5 |
...
| 35 | The v2/v3 ImageEncoder SIGSEGV was a **1-byte over-read into a Scudo guard page, fixable by padding the blob** — and, prerequisite to that, Genie **feeds the image file to the graph verbatim**, so the blob must be tensor-native UFixed16 bytes | `CLAUDE.md` hard contract (4096-byte pad), `preprocess_image.py` docstring, `NOTES-genie-pipeline.md` D1 parenthetical, `DEVICE_TEST_qwen3vl_imgenc_sigsegv.md` §1.2b, `DEVICE_PROBE_imgenc_page_alignment.md` | Both layers were wrong. `setupInput<DType>` (nsp-image-model.cpp:501-524) branches on `embedding-datatype`, which defaults to `QNN_DATATYPE_FLOAT_32` with **no config route for an image-encoder node** — the runtime reinterprets the file as float32 and reads `numElements*4` bytes, so the UFixed16 blob caused a **~3 MB over-read** (why +4 KB / +12 KB padding and mmap all failed, and why the fault always sat at the page-rounded buffer end). The fix is to ship **float32 pixel values** (`*_fp32.raw`, 6,291,456 B payload); Genie quantizes on device with the tensor's own scale/offset, reproducing the host quantizer byte-for-byte (v4 lint cross-checks fp32↔u16 at 0 LSB). The 4 KB pad is retained but inert. `qnn-net-run` *does* copy the file verbatim — the T5 result that vindicated the ViT is untouched. See `NOTES-genie-pipeline.md` D1b, `BUNDLE_V4_CHANGES_qwen3vl.md` §1. |
| 36 | The **prefill→decode KV handoff across the 2048→2175 width change** is the top suspect for the Qwen3-VL-4B decode defect — "Genie must re-lay 20 committed positions across that change. Nothing has ever tested it" | `reports/qwen3vl-4b-testi-device-results-2026-08-21.md` "What is actually left" (ranked #1); this file §0 until 2026-08-21; `TEST_J_decode_step.md` decision table | **Something has tested it: our own shipping 0.6B.** `qwen3-0.6b-w8a16-gqafix-ladekv` makes the identical handoff — prefill `past_key_0_in [1,8,128,1024]` → decode `[1,8,128,1151]`, `PAST` as the fastest axis, so the restride hazard is the same — on the same libGenie 1.19, and it generates correct coherent text in **basic** mode (prefill + decode only, the VL-4B's exact mode), token-exact against the LADE run for the first 60 tokens (`reports/qwen3-0.6b-w8a16-ladekv-test-report.md` §5). Read the widths back from `work/ctxbin/*/info.json`, not from the DLC names. Withdrawn; the surviving candidates are the **external-LUT feed** and the **two-ctx-bin split**. Ranking a hypothesis "never tested" is only sound after checking whether a *working* build already exercises it. |
| 40 | The Qwen3-VL-4B's saved KV cache is **~35× smaller than expected**, so it must be compressed, layer-subset or an opaque HTP handle | Test N device report §N1c, 2026-08-22 | It is the **full, raw, uncompressed** cache. The estimate used `36 × 2 × **28 heads** × 128 × 2 = 516,096 B/position` — the **query** head count. This is GQA: the cache is sized by the **8 KV heads** (`past_key_0_in [1,8,128,2048]`, read back from the shipped bin), giving `36 × 2 × 8 × 128 × 2 =` **147,456 B/position**, exactly the measured figure. The whole layout follows from the two file sizes and matches to the byte: 592 B header (16 B + 72 descriptors × 8 B), then 72 tensors (36 layers × K,V) of `n_pos × 2048 B`. ⇒ `--save` is a **live diagnostic lever**, not a dead end: P20 position 20 is decode-written and P21 position 20 is prefill-written for the same token, so diffing the two dumps measures the decode step's KV write directly (`scripts/validate/parse_genie_kv_dump.py`). Whenever a KV size looks wrong on a GQA model, check which head count went into the estimate. |
| 41 | The 2-ctx-bin **split** is the confirmed root cause of the 4B decode defect (N1a all-correct × Test M broken) | Test N device report, Executive Summary and "Combined Verdict", 2026-08-22 | **Not supported, and it contradicts N1a.** Prefill traverses BOTH shards (`prefill_0 → last_hidden_states → prefill_1`), crossing the same split decode uses — and N1a proves that handoff is correct on the 4B at six prompt lengths. Test M's 0.6B split meanwhile fails **at token 0**, i.e. in prefill, which is a *different* failure from the 4B's *first-token-right-then-degrades*. `TEST_M_split_reproduction.md` §2 assigns "wrong from token 0" to **"a prefill/feed problem, not the split"** — that is the row the session landed on, read as the row above it. Test M is further confounded: its config inherited the single-bin probe's engine block, which differs from the 4B's in four knobs, including **`enable-graph-switching: true`** on a two-ctx-bin model. ⚠ This does **not** exonerate the split for the 4B — decode uses a different graph pair and boundary shape (`[1,1,2560]`), so a decode-only handoff bug stays the leading hypothesis. It is simply not established. |
| 39 | Test K's LUT-probe run shows something about **Genie's LUT feed** — first the device team's "the feed is sound", then the build side's opposite "the feed is newly implicated" | `reports/qwen3vl-4b-testk-device-results-2026-08-21.md` §5 (both readings); this file §0 until 2026-08-22 | **Neither. It measured our own packaging.** The bundle published as `qwen3_06b_lutprobe/` shipped the **pre-graft** ctx-bin, `inputs_embeds` = `QNN_DATATYPE_FLOAT_16`, which **fails `lint_embedding_dtype.py`**; the corrected build (`9720e46e…`, `UFIXED_POINT_16`, scale 8.300073e-06 / offset −38356) passes. FLOAT_16 is exactly the dtype for which `quantizeInput` advances by `tensorOffset` **bytes** while `setupInputEmbeddings` passes **elements** (`nsp-model.cpp:3144` vs `:1813`), so the pad write for a partial prefill chunk overwrites the back half of the real prompt. It fires when `variant > n_process`, and the probe's prompts are 12/5/6 tokens in an **AR=128** graph — every one. That is the whole symptom: front of the prompt survives (`2 + 2 = 4` still appears), back is destroyed (the weather prompt becomes New York), decode untouched (AR=1 ⇒ `variant == n_process`) so 64 coherent tokens follow. ⚠ Two process failures, not one: (a) the v5 session had shipped **both** arms, correctly labelled `01_probe_06b_fp16in/` and `02_probe_06b_u16in/`, and the standalone folder was cut from the **fp16** arm with the label dropped; (b) my "fp16 eliminated" check read `work/ctxbin/…/info.json` — the **rebuilt** bin — while the device ran the **staged** copy. Verify the artefact you shipped, not one that shares its name. |
| 38 | `qwen3_06b_w8a16_gqafix_ladekv` is the **one-variable control** for the 0.6B LUT probe — "everything else is identical to the model that works" | `TEST_K_lut_vs_split.md` §K1b as first written (2026-08-21); `DEVICE_TEST_qwen3_06b_lut.md` §2's shape table, which compares against an unnamed "shipping" bin | It is not. The probe is **CTX 640** (`prefill mask [1,128,640]`, past 512; `decode [1,1,640]`, past 639); `gqafix-ladekv` is **CTX 1152** (past 1024 / 1151). The shape-matched control — identical in every dimension, `input_ids` vs `inputs_embeds` the only difference — is **`gqafix-cl512-ladekv`**, on the Hub at `2026-08-16-regime/bundles/qwen3_06b_w8a16_gqafix_cl512_ladekv.tar.gz`. The device team ran the CTX-1152 bundle as instructed. It does not invalidate the text comparison (mask width does not change greedy output) but it did produce one false inference: the probe's ~2× prefill rate is the **640 vs 1152 mask width**, not "LUT-fed prefill skips the embedding lookup" (a Gather is negligible). Read graph shapes back from `work/ctxbin/*/info.json` before calling any pair one-variable. |
| 37 | `genie-t2t-run` accepts **`--max-num-tokens`**, and it must always be passed so the profile flushes | `DEVICE_SESSION_PROTOCOL.md` §3/§5, `RUNBOOK_e2e_qwen3vl_4b.md` §2/§5, `DEVICE_TEST_qwen3_06b_lut.md` §3, all until 2026-08-21 | **No such flag exists.** `genie-t2t-run/main.cpp` accepts `-c/--config`, `-p/--prompt`, `--prompt_file`, `-tok/--tokens_file`, `-e/--embedding_file`, `-t/--embedding_table`, `--profile`, `-s/--save`, `-r/--restore`, `-w/--rewind`, `--action`, `--lora`, `--allow_engine_switch`, `--add_example_sampler`, `--embedding_query_output_type`. The token cap is the **dialog-config key `dialog.max-num-tokens`** (whitelisted `Dialog.cpp:2493`, read `:2888`), as the SDK's own example configs set it. The underlying advice was right — without a cap the query ends in `Context Size was exceeded`, which frees the handle and writes no profile — but the mechanism was wrong, and the Test J session lost all 4 profiles to it. Also learned: **`--profile` aborts if the output file already exists** (`main.cpp:528-533`), so every re-run needs a fresh name. |
