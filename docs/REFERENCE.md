...
| ↳ what is actually left | The difference between `r3_decodectx` (device, correct) and Genie's decode step 1 (device, wrong) is **who supplies the inputs**, not the graph: (1) the **prefill→decode KV handoff** — prefill's cache is `[1,8,D,2048]` and decode's `[1,8,D,2175]`, and Genie must re-lay the committed positions across that change; never tested. (2) the **LUT lookup of a generated token** at decode time (prompt-token lookup is proven by prefill being right). (3) mask/position advance on the first decode step |
| ↳ next test | `r3_decodectx` with **this** prompt: prefill the 20 templated tokens, seed the decode cache, one decode step under `qnn-net-run`. Expected argmax **151645**. Returns 151645 ⇒ Genie's plumbing is at fault and the bisect moves inside Genie; returns 2939 ⇒ the ctx-bin decode path, and the difference is cache contents. Small build — `--suite r` already does this, only the token list changes |
