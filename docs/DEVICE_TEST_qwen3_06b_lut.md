...
`qwen3-0.6b-w8a16-lutprobe` is the **same recipe as the working 0.6B**
(`full_build.sh <name> 128 512 --grouped-gqa`) with exactly one thing added:
`--input-embeds`. That changes three coupled things, which is why the flag is
detected in `full_build.sh` rather than only passed to the quantizer:
...
|---|---|---|
| **Coherent** ("4", a sensible paragraph) | the external-LUT feed path is **sound** on this runtime | the feed is exonerated. The 4B fault is then the split, or something 4B-specific. Build the 0.6B 2-way split probe next |
