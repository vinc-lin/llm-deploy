...
| | |
|---|---|
| **Next test** | **Test O — the FINAL exploratory session.** `TEST_O_variable_matrix.md` + `DEVICE_SESSION_PROTOCOL_O.md`, kit `qwen3vl_4b_testo_session/`. Every live variable (L1–L8) has a stage and a convicting signature; every branch of the decision tree ends in either **the model working end-to-end (Stage O6, same session)** or a named Genie mechanism with a complete Qualcomm escalation package. O1 = four KV dumps (the readout for 6 variables at once), O2 = the 7-config knob sweep with `run_o2_sweep.sh` printing PASS/FAIL inline, O3 = Test M deconfounded, O6 = the e2e run on a passing knob. Superseded ask: (1) send the two `state_p*.bin` KV dumps — `parse_genie_kv_dump.py --diff` measures the decode step's KV write directly, no device time. **(2) re-run Test M with the 4B's `QnnHtp` block** — config-only; Test M's config differs in 4 knobs incl. `enable-graph-switching: true` on a 2-bin model. Prior plan: **Test N** — `TEST_N_final_e2e.md` + `DEVICE_SESSION_PROTOCOL_N.md`. The complete remaining-work session (~60 min): **N1 probes the real 4B directly** (token ladder via `-tok`, `-e`, `--save` state dump — no build, runs on the bundle already on the board), with Tests **L** and **M** folded in as stages N2/N3, the image first-word grid as N4, timing completion as N5, and the post-fix e2e confirmation as N6. Kit: `qwen3vl_4b_testn_session/` (VL repo). If time is short: **N1a P21 is one command and the sharpest single measurement available** |
...
| **M** | 08-22 | does the 2-ctx-bin **split** break decode at 0.6B? | ⚠ ran, but **confounded** — failed at token 0 (a *prefill* signature, not the 4B's) with 4 engine knobs differing from the 4B (correction #41). Deconfounded re-run = Test O stage O3 | `TEST_M_split_reproduction.md` |
| **O** | **open** | **the final exploratory session** — variable matrix closed: KV-dump readout (L1–L6), knob sweep (L7), M deconfounded (L8), e2e gate on any pass | — | `TEST_O_variable_matrix.md` · `DEVICE_SESSION_PROTOCOL_O.md` |
