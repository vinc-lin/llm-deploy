...
>
> 1. **Its inputs are CORRECT — an earlier claim here that they were defective
>    is withdrawn.** All 60 files match the gqafix decode graph byte-for-byte
>    (`verify_profile_inputs.py`, correction #32). The 128 is `head_dim` and the
>    64 is `rope_dim`. The P1 cycle profile did fail, but not for this reason,
>    and regenerating the inputs fixes nothing.
...
>    ~18 tok/s where 44.707 was measured (`REFERENCE.md` §6.10, correction #27).
>
