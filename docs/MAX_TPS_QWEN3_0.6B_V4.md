...
| ~~A1.3~~ | **KV-quantization kernel check — ✅ DONE 2026-08-16, and the answer is yes** | A3.11 is **alive** | Written up in `docs/NOTES-htp-kernel-table.md`. v81 has exactly one mixed float×fixed MatMul kernel — **`f16 × sfxp8 -> f16`, with `input_quant_params` unconstrained on input1**, so a per-tensor-quantized *dynamic* tensor is legal there. `REFERENCE.md` §4.1's "unconfirmed kernel path" is confirmed; HTP-doc open question 8 is answered on the kernel half. Two hard constraints fall out: the KV cache must be **signed** INT8 (there is no `f16 × ufxp8`, and AIMET defaults activations to *unsigned*), and **INT16 KV is unreachable** while Q/attn stay FP16 — no `f16 × *fxp16` kernel exists. See A3.11 |
...
| ~~A1.5~~ | **Regenerate `decode_profile_inputs` — ❌ NOT THE BUG. Audited 2026-08-16** | P1 is still blocked, cause unknown | **The shipped package was already correct.** The pre-fix and post-fix decode-only ctx-bins have **byte-identical input contracts** — 60 inputs, same names, shapes and dtypes (the GQA fix is graph-internal; KV I/O was frozen by design), so a shape mismatch between them was *impossible*. `verify_profile_inputs.py` scores the shipped `ar1_decode/inputs` **60/60 against the gqafix bin**, including the `position_ids_cos` file the report called wrong at "128 bytes" — 128 B *is* `[1,1,64]` fp16, exactly right. The 08-15 §5 diagnosis, and the §0.3 annotation and rev-1 A1.6 that repeated it, are all withdrawn. **Ship `verify_profile_inputs.py` in the package** so the next failure names itself instead of being guessed at; re-run P1 with the environmental checklist in its docstring |
...
| **A3.8** | **`gqafix_cl512_ladekv`**, then `cl768` | **Discriminator arm 2** (§2.3): +10.1% byte vs +26.0% compute (measured) — the opposite ordering. New conversions, same post-B encodings lineage. Doubles as the product variant | Low |
...
- **12** (*is quantized KV reachable?*) — ✅ **CLOSED 2026-08-16: yes, as signed
  INT8 only** (`NOTES-htp-kernel-table.md`). Not a Genie-contract question and not
  an FAE question; it was a kernel-table lookup all along.
