...
| **qh ≈ 0%** (or negative) | — | **Expected, and now the leading hypothesis.** `REFERENCE.md` §8.1: the head's saving may never reach the device. The 2026-08-12 build shrank its DLC 151.3 MB but its ctx-bin only 12.5 MB, and **this drop's qh bin independently reproduces it — DLC −151 MB, converter `read_total_bytes` −146 MB, but ctx-bin only −8.4 MB.** The likely cause is HTP re-materializing the INT8 head to 16 bits at prepare time, since the `FullyConnected`'s input and output are both `Float_16` | Not a wasted build — it confirms §8.1 and closes a two-session-old question. But it means the byte model is **untestable on this arm**, so `cl512` carries the whole discrimination. Park `qh` permanently and read the verdict off `cl512` + `hvx8` alone |
