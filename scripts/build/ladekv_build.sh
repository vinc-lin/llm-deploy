...
# Same contract as full_build.sh: an embeddings-fed inputs_embeds must not land
# FLOAT_16, because quantizeInput's FLOAT_16 case advances by BYTES where its
# caller passes ELEMENTS. Must be handled HERE too -- this script converts the
# past-KV prefill separately, so an encoding grafted only in full_build.sh would
# ship a correct decode graph beside a prefill still declaring FLOAT_16.
GRAFT_EMBEDS=0
